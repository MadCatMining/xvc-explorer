...
//var base_url = 'http://127.0.0.1:3500/api/';
  ...
  get_hashrate: function(cb) {
    if (settings.use_rpc) {
        var headers = {
         ...
         var dataString = '{"jsonrpc": "1.0", "id":"curltest", "method": "getnetworkhashps"}';
         var options = {
         ...
         function callback(error, response, body) {
             if (!error && response.statusCode == 200) {
                ...
                // console.log("Net Hashrate :", obj.result);
                 return cb((obj.result / 1000000000).toFixed(4));
          }
        }
  ...
  get_difficulty: function(cb) {
    if (settings.use_rpc) {
        var headers = {
         ...
         var dataString = '{"jsonrpc": "1.0", "id":"curltest", "method": "getdifficulty"}';
         var options = {
             url: 'http://127.0.0.1:8740/',
             method: 'POST',
             headers: headers,
  ...
  get_connectioncount: function(cb) {
    if (settings.use_rpc) {
        var headers = {
         ...
         var dataString = '{"jsonrpc": "1.0", "id":"curltest", "method": "getinfo"}';
         var options = {
             url: 'http://127.0.0.1:8740/',
             method: 'POST',
             headers: headers,
         ...
         function callback(error, response, body) {
             if (!error && response.statusCode == 200) {
                 const obj = JSON.parse(body);
                 //console.log(obj.result);
                 return cb(obj.result.connections);
             }
         }
         request(options, callback);
    } else {
 ...
 get_blockcount: function(cb) {
   if (settings.use_rpc) {
       var headers = {
        ...
        var dataString = '{"jsonrpc": "1.0", "id":"curltest", "method": "getblockcount"}';
        var options = {
            url: 'http://127.0.0.1:8740/',
            method: 'POST',
            headers: headers,
  ...
  get_blockhash: function(height, cb) {
    if (settings.use_rpc) {
        var headers = {
         ...
         console.log(height);
         //var dataString = "{"jsonrpc": "1.0", "id":"curltest", "method": "getblockhash", "params":[height]}";
    ...
    if (settings.use_rpc) {
        var headers = {
         ...
         function callback(error, response, body) {
             if (!error && response.statusCode == 200) {
                 //console.log(body);
                 //console.log('GetRawTransaction Call')
                 //const obj = JSON.parse(body);
                 return cb(body.result);
             }
         }
    ...
    if (settings.use_rpc) {
      if ( settings.supply == 'HEAVY' ) {
        ...
        function callback(error, response, body) {
            if (!error && response.statusCode == 200) {
                const obj = JSON.parse(body);
                return cb(obj.result.moneysupply);
            }
        }
        request(options, callback);
